 ...
 *
 * Module which encapsulates logic for update ToDo endpoint.
...
var UpdateToDoEndpoint = function()
{
  // Global module level variables.
  var MODULE_NAME = "UpdateToDoEndpoint.js: ";
  ...
  function updateToDo(requestParams, callback)
  {
    ...
    [
      ...
      function doValidateUpdateTodoRequest(sessionObject, cb)
      {
        // Validate request parameters.
        var validationResponse = validation.validateUpdateTodoRequest(requestParams);
      ...
      function doUpdateToDo(sessionObject, cb)
      {
        // Construct update TODO details object.
        var params =
        {
          "userId": sessionObject.userId,
          "toDoId": requestParams.request.payload.updateToDo.toDoId,
          "description": requestParams.request.payload.updateToDo.description,
          "deadline": requestParams.request.payload.updateToDo.deadline,
          "latitude": requestParams.request.payload.updateToDo.latitude,
          "longitude": requestParams.request.payload.updateToDo.longitude,
          "status": requestParams.request.payload.updateToDo.status,
          "note": requestParams.request.payload.updateToDo.note
        };

        // Update TODO details from database.
        var updateToDoResponse = toDoDAO.updateToDo(params, function handleUpdateToDoResponse(error, responseJson)
        {
          // If update operation failed return failure callback.
          if (error)
          {
            var errorResponse = responseUtils.constructErrorResponseJson(constants.RESP_BAD_INPUT, "Update ToDo", "Operation Failed", error.message);
            return callback(errorResponse, null);
          }

          var response = responseUtils.constructStatusResponseJson("updateToDo", constants.RESP_SUCCESS, "ToDo updated successfully.");
          cb(null, response);
        });
      }
...
// Export module.
module.exports = new UpdateToDoEndpoint();