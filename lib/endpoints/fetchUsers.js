 ...
 *
 * Module which encapsulates logic for fetch users endpoint.
 * - Extracts user id from session object.
 * - Calls a data layer method to fetch list of users.
  ...
  function fetchUsers(requestParams, callback) {
    ...
    async.waterfall(
      [
        ...
        function doFetchUserList(sessionObject, cb) {
          // Construct object to fetch userlist.
          var params = {
          ...
          var fetchUsersResponse = userDAO.fetchUserList(params, function handleFetchUserListResponse(error, responseJson) {
            // If list operation failed return failure callback.
            if (error) {
              var errorResponse = responseUtils.constructErrorResponseJson(constants.RESP_BAD_INPUT, "Fetch Users", "Operation Failed", error.message);
              return callback(errorResponse, null);
            }

            var response = responseUtils.constructStatusResponseJson("fetchUsers", constants.RESP_SUCCESS, "All Users fetched successfully.");
            response.response.payload.fetchUsers.userList = responseJson;
            cb(null, response);
          });
        }
...
// Export module.
module.exports = new FetchUsersEndpoint();